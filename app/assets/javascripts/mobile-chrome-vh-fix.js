var VHChromeFix = function(selectors) {
  ...
  var isSamsung = /samsungbrowser/.test(userAgent);
  ...
  if (isSamsung){
    ...
    window.addEventListener('resize', function() {
      ...
      // This is a hack, as Android when eyboard pops up
      // Triggers orientation change
      if (self.windowWidth !== document.body.clientWidth && self.windowHeight !== document.body.clientHeight) {
        self.windowWidth = document.body.clientWidth;
        self.windowHeight = window.document.body.clientHeight;
        self.fixAll();
      }
    });      
  }
...
VHChromeFix.prototype.fixAll2 = function() {
  for (var i = 0; i < this.elements.length; i++) {
    var element = this.elements[i];
    element.domElement.style.height = (document.body.clientHeight * element.vh / 100) + 'px';
  }
